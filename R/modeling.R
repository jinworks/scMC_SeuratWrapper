...
identifyClusters_internal <- function(object, resolution = NULL, method = "matrix", algorithm = 4, resRange = NULL, nDims.consensus = 30, clustering.method = c("hierarchical", "community"), graph.name = NULL,...) {
  ...
  if (!is.null(resolution)) {
    ...
    # X = 1:length(resRange),
    # FUN = function(x) {
    #   object0 <- FindClusters(object, resolution = resRange[x], method = method, algorithm = algorithm, verbose = FALSE, ...)
    #   idents <- Idents(object0)
    ...
    # ncluster <- unlist(lapply(results, "[[", 2))
    # rm(results)
...
#'
identifyIntegrationFeatures <- function(object.list, integrationFeatures.method = c("joint", "individual"), selection.method = c("vst", "mean.var.plot"), nfeatures = 3000, mean.cutoff = c(0.01, 5), dispersion.cutoff = c(0.25, Inf), ...) {
  ...
  } else (integrationFeatures.method == "seurat")
...
RunscMC <- function(object.list,
                    ...
                    integrationFeatures.method = c("joint", "individual"), selection.method = c("vst", "mean.var.plot"), nfeatures = 3000, mean.cutoff = c(0.01, 5), dispersion.cutoff = c(0.25, Inf), # parameters in identifyIntegrationFeatures
